...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $hireDate = $_POST['hireDate'] ?? $staff['HireDate'];
    ...
    if (empty($firstName) || empty($lastName) || empty($role) || empty($email) || 
        ...
        // Check if username or email already exists for other staff
        $checkQuery = "SELECT StaffID FROM Staff WHERE (Username = ? OR Email = ?) AND StaffID != ?";
        $stmt = $conn->prepare($checkQuery);
        $stmt->bind_param("ssi", $username, $email, $staffId);
        $stmt->execute();
        $result = $stmt->get_result();
        
        if ($result->num_rows > 0) {
            $error = 'Username or Email already exists for another staff member!';
        } else {
            // Update staff
            $updateQuery = "UPDATE Staff SET FirstName = ?, LastName = ?, Role = ?, Email = ?, Phone = ?, 
                            HireDate = ?, IsActive = ?, Address = ?, ICPassportNo = ?, Username = ?, Password = ?
                            WHERE StaffID = ?";
            $stmt = $conn->prepare($updateQuery);
            $stmt->bind_param("ssssssissssi", $firstName, $lastName, $role, $email, $phone, 
                              $hireDate, $isActive, $address, $icPassport, $username, $password, $staffId);
...
<head>
  <meta charset="UTF-8">
  <title>Edit Staff | Restoran Badhriah</title>
...
<div class="main">
  <h1>Edit Staff: <?= htmlspecialchars($staff['FirstName'] . ' ' . $staff['LastName']) ?></h1>
    ...
    <form method="POST">
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="text" id="firstName" name="firstName" value="<?= htmlspecialchars($staff['FirstName']) ?>" required>
        </div>
        
        <div class="form-group">
          <label for="lastName">Last Name</label>
          <input type="text" id="lastName" name="lastName" value="<?= htmlspecialchars($staff['LastName']) ?>" required>
        </div>
      </div>
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <input type="date" id="hireDate" name="hireDate" value="<?= htmlspecialchars($staff['HireDate']) ?>" required>
        </div>
      </div>
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="text" id="icPassport" name="icPassport" value="<?= htmlspecialchars($staff['ICPassportNo']) ?>" required>
        </div>
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <div class="checkbox-group">
            <input type="checkbox" id="isActive" name="isActive" <?= $staff['IsActive'] ? 'checked' : '' ?>>
